import { execute, generateId } from './db.js';
...
async function seedPaymentMethods() {
  const paymentMethods = [
    { id: 'mada', name: 'Mada', name_ar: 'مدى', icon: 'credit-card', display_order: 1 },
    { id: 'apple-pay', name: 'Apple Pay', name_ar: 'أبل باي', icon: 'smartphone', display_order: 2 },
    { id: 'google-pay', name: 'Google Pay', name_ar: 'جوجل باي', icon: 'smartphone', display_order: 3 },
    { id: 'cash', name: 'Cash on Delivery', name_ar: 'الدفع عند الاستلام', icon: 'banknote', display_order: 4 },
  ];

  for (const pm of paymentMethods) {
    await execute(
      `INSERT INTO payment_methods (id, name, name_ar, icon, display_order)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (id) DO NOTHING`,
      [pm.id, pm.name, pm.name_ar, pm.icon, pm.display_order]
    );
  }
...
async function seedProducts() {
  const products = [
    ...
    // Snacks
    { id: '16', name: 'Chips & Snacks', name_ar: 'رقائق ووجبات خفيفة', category: 'Snacks', price: 16.50, barcode: '6006234567895', weight: null },
    // Meat & Poultry
    { id: '17', name: 'Fresh Chicken', name_ar: 'دجاج طازج', category: 'Meat & Poultry', price: 42.00, barcode: '6007234567896', weight: 1.5 },
    // Clothing
    { id: '18', name: 'Cotton T-Shirt', name_ar: 'قميص قطني', category: 'Clothing', price: 75.00, barcode: '6008234567897', weight: null },
    { id: '19', name: 'Casual Pants', name_ar: 'بنطال كاجوال', category: 'Clothing', price: 150.00, barcode: '6008234567903', weight: null },
    // Kitchen
  ...
  for (const p of products) {
    await execute(
      `INSERT INTO products (id, name, name_ar, category, price, barcode, weight, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, $7, true)
       ON CONFLICT (id) DO NOTHING`,
      [p.id, p.name, p.name_ar, p.category, p.price, p.barcode, p.weight]
    );
  }
...
// Export individual seeders for flexibility
export { seedPaymentMethods, seedProducts };
